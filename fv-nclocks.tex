...
A \emph{valuation sequence} over inputs $I$, outputs $O$, and timers $Y \subseteq X$ is an alternating sequence
\begin{eqnarray*}
...
To each valuation sequence $\sigma$ we can associate a corresponding behavior by forgetting the time
delays and by replacing valuations by their domain:
\begin{eqnarray*}
\beh(\sigma) & = & \domof{\tvals_0} \xrightarrow{i_1/o_1, \rho_1} \domof{\tvals_1}  \xrightarrow{i_2/o_2, \rho_2} \domof{\tvals_2} \cdots \xrightarrow{i_k/o_k, \rho_k} \domof{\tvals_k}.
\end{eqnarray*}
We say that behavior $\beta$ is \emph{feasible} if there exists a valuation sequence $\sigma$ such that $\beh(\sigma) = \beta$.
We can also associate a timed word to valuation sequence $\sigma$ by forgetting the valuations and the initialization functions:
\begin{eqnarray*}
...
\begin{eqnarray*}
\pi_1 (\alpha) & = & q_0 \xrightarrow{i_1/o_1, \rho_1} q_1  \xrightarrow{i_2/o_2, \rho_2} q_2 \cdots \xrightarrow{i_k/o_k, \rho_k} q_k
\end{eqnarray*}
(note that the $\rho_j$ are uniquely determined since $\M$ is deterministic) and to a valuation sequence
\begin{eqnarray*}
\pi_2(\alpha) & = & \tvals_0 \xrightarrow{d_1} \tvals'_0 \xrightarrow{i_1/o_1, \rho_1} \tvals_1 \xrightarrow{d_2} \tvals'_1 \xrightarrow{i_2/o_2, \rho_2} \tvals_2 \cdots
\xrightarrow{d_k} \tvals'_{k-1} \xrightarrow{i_k/o_k, \rho_k} \tvals_{k}.
\end{eqnarray*}
Note that $\beh(\pi_1(\alpha))=\beh(\pi_2(\alpha))$ and $\timedword(\alpha) = \timedword(\pi_2(\alpha))$.
Conversely, if $\gamma$ is an untimed run  of $\M$ and $\sigma$ is a valuation sequence such that $\beh(\gamma) = \beh(\sigma)$,
then there exists a unique timed run $\alpha$ of $\M$ with $\pi_1(\alpha) = \gamma$ and $\pi_2(\alpha) = \sigma$.
We refer to $\alpha$ as $\run(\gamma,\sigma)$.
\marginpar{Add a commuting diagram to summarize.}

\subsection{Untimed semantics}
We would, intuitively, like to let the untimed semantics of an MMT $\M$ be the set of its feasible behaviors.
However, it would then depend heavily on the names of the timers. Therefore, we define an equivalence relation
on behaviors, which deems two behaviors equivalent if there is a consistent renaming of timers that transforms
one behavior into the other.
...
\beta & = & X_0 \xrightarrow{i_1/o_1, \rho_1} X_1  \xrightarrow{i_2/o_2, \rho_2} X_2 \cdots \xrightarrow{i_k/o_k, \rho_k} X_{k}
\end{eqnarray*}
is a behavior then $f(\beta)$ is defined as the behavior
\begin{eqnarray*}
...
(The reader may check that $f(\beta)$ is a behavior indeed.)
Two behaviors $\beta$ and $\beta'$ are \emph{isomorphic} if there exists an isomorphism $f$ such that
$\beta' = f(\beta)$.
Two sets of behaviors $A$ and $B$ are \emph{isomorphic} if for each behavior of $A$ there is an isomorphic behavior in $B$,
and vice versa.

Isomorphisms are lifted to valuation sequences in the obvious way. If $f = f_0 ,\ldots, f_k$ is an isomorphism and
\begin{eqnarray*}
\sigma & = & \tvals_0 \xrightarrow{d_1} \tvals'_0 \xrightarrow{i_1/o_1, \rho_1} \tvals_1 \xrightarrow{d_2} \tvals'_1 \xrightarrow{i_2/o_2, \rho_2} \tvals_2 \cdots
\xrightarrow{d_k} \tvals'_{k-1} \xrightarrow{i_k/o_k, \rho_k} \tvals_{k}
\end{eqnarray*}
is a valuation sequence with $\domof{\tvals_j} = \domof{\tvals'_j} = \domof{f_j}$, for all $j$, then $f(\sigma)$ is
...
\end{eqnarray*}
where $i'_j$ and $\rho'_j$ are defined in the same way as for behaviors, and
$\lambda_j = \kappa_j \circ f_j^{-1}$ and $\lambda'_j = \kappa'_j \circ f_j^{-1}$, for all $j$.
Two valuation sequences $\sigma$ and $\sigma'$ are \emph{isomorphic} if there exists an isomorphism $f$ such that
$\sigma' = f(\sigma)$.
Note that isomorphic valuation sequences induce the same timed words.

The following lemmas follow directly from the definitions:
\begin{lemma}
\label{lemma isomorphism}
Let $\sigma$ be a valuation sequence and let $f$ be an isomorphism for $\sigma$.
Then $\beh(f(\sigma)) = f(\beh(\sigma))$.
\end{lemma}
\begin{lemma}
If $\beta$ and $\beta'$ are isomorphic, then $\beta$ is feasible iff $\beta'$ is feasible.
\end{lemma}
...
Let $\sigma = \pi_2(\alpha)$ and $\beta = \beh(\sigma)$. 
Then $\beta$ is a feasible behavior of $\M$ and $\timedword(\sigma) = w$.
Since  $\M \approx_{\mathit{untimed}} \N$, there exists an isomorphism $f$ such that 
$\beta' = f(\beta)$ is a feasible behavior of $\N$.
Hence $\N$ has an untimed run $\gamma'$ such that $\beh(\gamma') = \beta'$.
Let $\sigma' = f(\sigma)$.
By Lemma~\ref{lemma isomorphism}, $\sigma'$ is a valuation sequence with 
$\beh(\sigma') = \beh(f(\sigma)) = f(\beh(\sigma)) = f(\beta) = \beta'$.
Since $\beh(\gamma') = \beh(\sigma') = \beta'$, $\N$ has a timed run $\alpha' = \run(\gamma',\sigma')$ with
$\pi_2(\alpha') = \sigma'$.
...
Let $\M$ be an MMT. Then we say that $\M$ is \emph{timer live} if, for each feasible behavior $\beta$ with $\Last{\beta} = Y$ and for each timer $x \in Y$, there exists a behavior $\beta'$ with $\Head{\beta'} = Y$ that only contains
...
Let $\beta$ be a feasible behavior of $\M$.
By induction on the number of events in $\beta$, we prove that $\N$ has a feasible behavior that is isomorphic to $\beta$.
...
Since $\emptyset$ is a feasible behavior of any MMT, $\beta = \emptyset$ is a feasible behavior of $\N$, as required.
...
\end{eqnarray*}
Let $\beta_k$ be the prefix of $\beta$ containing $k$ events. Then $\beta_k$ is also a feasible behavior and thus, by
induction hypothesis, there exists a feasible behavior
\begin{eqnarray*}
...
\begin{lemma}
Suppose $\beta$ is a feasible behavior and $Y \xrightarrow{i/o, \rho} Y'$ is a behavior,
with $Y = \Last{\beta}$ and $i \in I$.
Then $\beta \xrightarrow{i/o, \rho} Y'$ is a feasible behavior.
\end{lemma}
...
\begin{lemma}
Suppose $\beta$ is a feasible behavior and $Y \xrightarrow{\toevent{x}/o, \rho} Y'$ is a behavior 
with $Y = \Last{\beta}$.
