...
programming and in the theory of timed automata, but nonzero delays are essential in our learning approach.
...
in which one has to slide blocking vehicles out of the way to find a path for one specific red car to exit
...
We may for instance slighly modify the timed behavior of Figure~\ref{fig:races} by scheduling block $\{ 2  \}$
a bit later and block $\{ 4, 5 \}$ a bit earlier.
Then all races have been eliminated and we can wiggle the timing of block $\{ 1, 3 \}$.
...
A block that does not win any race can be wiggled forward by a small amount (if you don't win you might as wel start later).
...
Once we have eliminated the bottom elements we can work our way upwards in the partial order and wiggle all the blocks
