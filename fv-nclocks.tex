...
they expire.
\marginpar{Give examples here; timer liveness is not enough! Need: at most one timer started on each transition, timer liveness.}
...
Suppose $i_q$ is of the form $\toevent{x}$. Then we say that event $i_p$ \emph{triggers} $i_q$ if
$p<q$, event $i_p$ starts timer $x$, and for all $p < r < q$, timer $x$ is unaffected by event $i_r$. 
A \emph{trigger sequence} of $\sigma$ is a maximal subset of indices $T = \{ p_1 ,\ldots, p_u \}$ such that $i_{p_1} \in I$, $i_{p_1}$ triggers $i_{p_2}$, $i_{p_2}$ triggers $i_{p_3}$, etc.
The following lemma allows us to shift all the events in a trigger sequence e by a small amount forward and backward, respectively, under the condition that all valuations are injective.
...
\end{eqnarray*}
\end{lemma}
Without the assumption of injectivity, Lemma~\ref{wiggle lemma} does not hold. 

\begin{example}
\label{no wiggling example}
Consider the following timed behavior with trigger sequences $\{ 1, 3 \}$ and $\{ 4, 5 \}$:
\[
...
Note that timer $y$ is stopped when timer $x$ expires.
\end{example}
...
\end{lemma}
\begin{proof}
If all valuations in $\sigma$ are injective then we set $\sigma' = \sigma$ and we are done.

So assume that $\sigma$ contains a valuation that is not injective. By induction on the smallest number
\end{proof}

\marginpar{Need to  reorder paper?}
