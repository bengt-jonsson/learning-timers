...
A \emph{timed learner} may perform \emph{timed experiments} to learn about the
...
%Let $\M = (I, O, Q, q_0, \vars, \delta, \lambda, \remap)$ be an MMT.
...
We want to reduce the problem of deciding untimed equivalence of transparent MMTs to the problem of deciding
...
The teacher knows the state diagram, which in our setting is a Mealy machine $\M$. 
Initially, the learner only knows the inputs $I$ of $\M$.
The task of the learner is to learn $\M$ through two types of queries:
\begin{itemize}
\item
With a \emph{membership query (MQ)}, the learner asks what the output is in response to an input sequence $\sigma \in I^{\ast}$.
The teacher answers with output sequence $A_{\M}(\sigma)$.
\item
With an \emph{equivalence query (EQ)}, the learner asks if a hypothesized Mealy machine $\CH$ with
inputs $I$ and outputs $O$ is correct, that is, whether $\CH$ and $\M$ are equivalent.
The teacher answers \emph{yes} if this is the case. Otherwise she answers \emph{no} and supplies a
