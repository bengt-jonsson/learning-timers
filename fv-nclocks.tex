...
\vspace{-2em}
\begin{center}
\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=2cm,main node/.style={circle,draw,font=\sffamily\large\bfsthe name of the eries}]
  \node[initial, state] (1) {$q_0$};
...
To each untimed run $\gamma$ we associate a corresponding untimed behavior by simply replacing all
...
To each timed behavior $\sigma$ we associate a corresponding untimed behavior by forgetting the
...
%
\else
Note that for an untimed behavior $\beta$, the set $\zone{\beta}$ of timer valuations that can be reached with a timed behavior $\sigma$ with $\untime(\sigma) = \beta$ can be symbolically represented and computed using Difference Bound Matrices (DBMs) \cite{Di89}.
