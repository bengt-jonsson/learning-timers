...
$u = i_1 \cdots i_k$.
Inductively, we construct a proper response for $u$.
\marginpar{Needs doing}
...
\marginpar{Here is still some minor loose ends What if the untimed membership query returns undefined? Also: if $\CH$ and $\M$ are not timed equivalent, how do we know there is a counterexample in which the fractional parts are different?}
