...
If a timed behavior has no races then, by Lemma~\ref{wiggle lemma},
we can wiggle the timing of all the blocks. We can do this in such a way that
the fractional part of the absolute times of any pair of events from different blocks is different.
We call a timed behavior without races \emph{transparent} if it has this property.
By Lemma~\ref{race elimination} it follows that for each feasible untimed behavior $\beta$ there exists
