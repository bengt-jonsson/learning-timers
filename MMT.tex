...
The MMT of Figure~\ref{fig:nondeterminism} (top), for instance, accepts both the timed words
...
\begin{center}
 \begin{tikzpicture}[>=stealth]
            ...
            \draw [<-] (1.5,2.5) -- (7,2.5) node[midway,below] {\small max timed word $w$ of $\M$ s.t.\  $\timedinputword(w) \propto u$};
...
there exists a maximal timed word $w$ of $\M$ such that $\timedinputword(w) \propto u$.
...
Note that, in general, the teacher may give more than one answer to a membership query.
In the example of Figure~\ref{fig:nondeterminism} (bottom), for instance, the membership query $7 ~ i ~ 1 ~ i ~ 1$ may result in both anwer
$7 ~ i ~ o ~ 1 ~ i ~ o ~ 1 ~ \mathit{to} ~ o'$ and answer  $7 ~ i ~ o ~ 1 ~ i ~ o ~ 1 ~ \mathit{to} ~ o''$.
...
\emph{counterexample}: a timed word $w$ of $\M$ that is not a timed word of $\CH$.
(Observe that such a timed word always exists.)
