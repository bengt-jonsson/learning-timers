...
$\getmemorable{S}{\beta}{V}$ be the set of timers $x_i$ in
$x_1 , \ldots x_{|\beta|}$ whose corresponding timeout event
(of form $\toevent{x_i}$) occurs (as an input) in some untimed behavior in
$\suffixbehs{S}{\beta}{V}$.
Let $\getassignment{S}{\beta}{V}$ map each timer $x_i$ in
$\getmemorable{S}{\beta}{V}$ to the unique positive integer to which it
...
$U$ is {\em timer-consistent} wrt.\ $V$ if 
  ...
  (\getmemorable{S}{\beta}{V} \cup \set{x_{(|\beta|+1)}})$.
\end{itemize}
...
\item $\remap$
 maps each $\beta \in U$ and $i \in \feasibleinputs{\beta}{S}$ to
  ...
  timer $x_{(|\beta|+1)}$ to
 $\getassignment{S}{\extend{\beta}{i}{\lambda(\beta,i)}}{V}(x_{(|\beta|+1)})$.
\end{itemize}
\end{definition}
...
for timer $x_j$ in
$\getmemorable{S}{\extend{\beta}{i}{\lambda(\beta,i)}}{V} \setminus (\getmemorable{S}{\beta}{V} \cup \set{x_{(|\beta|+1)}})$, find an 
