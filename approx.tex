...
\todobj{Got this far. Will continue here}
%% \begin{itemize}
%%   \item 
%%     During {\em hypothesis construction}, membership queries are performed for
%%     all untimed input words in $UV \cup U\alphabet V$.
%%     The purpose of this is to compute the relation
%%     $\apprnerodeof{\Lang}{V}$ on $U \cup U\alphabet$.
%%     Whenever the set $U$ is not closed wrt.\ $V$, then it is extended:
%%     if there is some
%%     $ua$ with $u \in U$ such that $ua \not\apprnerodeof{\Lang}{V} u'$ for all $u' \in U$, then $ua$ is added to $U$,  triggering new membership queries. The
%%     extension of $U$ is continued in this way until $U$ is closed wrt.\ $V$.
%%   \item
%%     When $U$ is closed wrt.\ $V$, then the hypothesis $\hypoof UV$ is
%%     validated by submitting it in an equivalence query.
%%     If the query returns ``yes'', then
%%     the learning is completed, and $\hypoof UV$ accepts $\Lang$.
%%     If the query returns a counterexample word $w$, this is used to extend
%%     $V$ as follows. By the fact that $w$ is a counterexample, there is a suffix
