...
it is interesting to see how easy it is to implement lookahead queries. For MMTs with a single timer this is trivial 
if we assume an upper bound $t$ on the value to which timer are set (just  wait for $t+1$ time units).
But are lookahead queries also easy to implement for practical applications with multiple timers?
