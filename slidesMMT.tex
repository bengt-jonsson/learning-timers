...
\begin{center}
 \begin{tikzpicture}[>=stealth]
            ...
            \draw [thick] (9.5,0) rectangle (11,3.5) node[midway] {Teacher};
            \draw [->] (1.5,3) -- (9.5,3) node[midway,below] {MQs};
            \draw (1.5,3) -- (9.5,3) node[midway,above] {\small transparent timed input word $u$};
            \draw [<-] (1.5,2.5) -- (9.5,2.5) node[midway,below] {\small maximal timed word $w$ with $\timedinputword(w) \propto u$};
            \draw [->] (1.5,1) -- (9.5,1) node[midway,below] {EQ};
            \draw (1.5,1) -- (9.5,1) node[midway,above] {\small hypothesis $\CH$};
            \draw [<-] (1.5,0.5) -- (9.5,0.5) node[midway,below] {\small {\bf yes} \emph{or} {\bf no}+transparent counterexample $w$};
        \end{tikzpicture}
\end{center}        
}
...
%\vspace{-1cm}
 \begin{tikzpicture}[>=stealth]
 \draw [dashed] (-0.1,-2.6) rectangle (6.1,3.6) node[right] {Untimed MMT teacher};
            \draw [thick] (0,0) rectangle (1.5,3.5) node[midway,below] {\small Teacher};
               \draw [thick] (0,0) rectangle (1.5,3.5) node[midway,above] {\small Timed};
            \draw [thick] (4.5,0) rectangle (6,3.5) node[midway] {\small Adapter};
            \draw [->] (1.5,3) -- (4.5,3) node[midway,below] {MQs};
            \draw [<-] (1.5,2.5) -- (4.5,2.5);
            \draw [->] (1.5,1) -- (4.5,1) node[midway,below] {EQs};
            ...
            \draw [->] (6,1) -- (7,1);
            \draw [<-] (6,0.5) -- (7,0.5);
            \draw [thick] (4.5,-2.5) rectangle (6,-1) node[midway,below] {\footnotesize Oracle};
            \draw [thick] (4.5,-2.5) rectangle (6,-1) node[midway,above] {\footnotesize Lookahead};
            \draw [->] (5.5,-1) -- (5.5,0) node[midway,right] {\small {\bf no} \emph{or} {\bf yes}+timeout value};
            \draw [->] (5,0) -- (5,-1) node[midway,left] {\small untimed input word $u$ + index $j$};
        \end{tikzpicture}
\end{center}
...
Number of queries polynomial in size canonical MMT $\N$ produced by Myhill-Nerode construction.

\vspace{1 em}
Note that this MMT may be exponentially bigger (in the number of clocks) than original MMT $\M$ of the teacher. (Similar to register automata.)

\vspace{1 em}
For MMTs with single timer, learning is easy: all untimed behaviors are feasible, lookahead oracle is trivial if we assume learner
