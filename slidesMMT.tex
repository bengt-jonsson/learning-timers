...
\frame{
\frametitle{Angluin's minimally adequate teacher}

\begin{center}
\includegraphics[width=.3\textwidth]{angluin.png}
\end{center}
}
\frame{
\frametitle{Black box checking}

\begin{center}
\includegraphics[width=.7\textwidth]{blackboxchecking.jpg}
\end{center}
\red{Learner}: Formulate hypothesis\\
\red{Tester}: Test correctness hypothesis
}
...
\begin{center}
\includegraphics[width=.4\textwidth]{TCPbug.png}
\end{center}
Standard violations found in implementations of major protocols, e.g.,
\blue{TCP}\  (CAV'16), \blue{TLS}\  (Usenix Security'15), \blue{SSH}\  (Spin'17)
}
...
\frame{
\frametitle{For more applications see CACM paper}
...
\blue{Timing behavior}\  plays a crucial role in applications of model learning, but existing tools cannot handle it. Today's tools only work for untimed, deterministic systems. \\
...
Develop learning algorithm for Mealy machines with timers in which transitions can be triggered by both input events and timeouts,
...
\begin{center}
\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=2.8cm,main node/.style={circle,draw,font=\sffamily\large\bfseries}]
...
\end{tikzpicture}
\end{center}
Diagram commutes and we have a \blue{pullback}.
...
\pause
\vspace{1 em}
$\Rightarrow$ Assume that at most one timer can be updated per transition.
...
\begin{block}{Definition}
An MMTs $\M$ is \red{timer live}\  if for each feasible untimed behavior $\beta$ and each timer $y$ active after $\beta$, 
there is a \red{witness}, an untimed behavior $\beta_y$  that leaves $y$ unaffected, except for the last transition in which $y$ expires, such that $\beta \cdot \beta_y$ is feasible.
\end{block}
...
\pause
\vspace{1 em}
Main proof technique: \blue{wiggling}\  of timed behaviors to ensure that fractional starting times of different inputs are different.
}
...
A timed word is \red{transparent}\  if the fractional part of the absolute times of all input events is different. 
...
\pause
A \red{causality map}\  for timed word $w$ is an injective function that maps each timeout to a preceding input such that
the time elapsed between the input and the timeout is an integer.
